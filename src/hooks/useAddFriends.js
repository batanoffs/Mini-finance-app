...
export const useAddFriend = () => {
    ...
    const validateRequest = useCallback(async () => {
        ...
        if (number === phone) return 'You cannot add yourself'
        ...
        if (friends?.some((friend) => friend.objectId === receiverId)) {
            return 'User is already your friend'
        }
        return { receiverId }
    }, [number, phone, friends])
    ...
    const sendFriendRequest = useCallback(
        ...
        [number, userDataId, token, showMessage]
    )
    ...
    const handleDeclinedRequest = useCallback(
        async (notification, receiverId, senderId) => {
            ...
            if (notification.receiver[0].objectId === userDataId) {
