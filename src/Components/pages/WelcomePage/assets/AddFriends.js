            ...
            if (checkFriendNotification.length === 0) {
                ...
                if (status === "declined") {
                    ...
                    if(receiverID === userDataId) {  
                        const setSenderResponse = await notificationService.updateRelation(notificatinID, "sender", receiverID, token);
                        const setReceiverResponse = await notificationService.updateRelation(notificatinID, "receiver", senderID, token);

                        console.log("setSenderResponse", setSenderResponse);
                        console.log("setReceiverResponse", setReceiverResponse);

                        if (!setSenderResponse || !setReceiverResponse || setSenderResponse === 0 || setReceiverResponse === 0) {
                            throw new Error("Failed to update relation");
                        }
                        const response = await notificationService.updateNotificationStatus(notificatinID,"pending", false, token);
                        if (!response) {
